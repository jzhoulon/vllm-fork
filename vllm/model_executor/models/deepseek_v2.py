...
class Indexer(nn.Module):
    ...
    def sparse_attn_indexer(
       ...
       # print("topk_indices_buffer shape = ", topk_indices_buffer.shape)
        ...
        if kv_cache is None:
            return
        '''
        x = torch.rand(1,4096,4096, device="hpu")
        print("x shape = ", x.shape)
        indices_cpu = x.to("cpu").topk(512, dim=-1)[1]
        print("sparse_attn_indexer indexer indices_cpu = ", indices_cpu)
        indices_hpu = x.topk(2048, dim=-1)[1].to("cpu")
        print("sparse_attn_indexer indexer indices_hpu = ", indices_hpu)
        '''
        ...
        if is_prefill:
            ...
            if kv_cache is not  None:
               ...
               #k2 = self.cache_op.fetch_from_cache(kv_cache, block_list).unsqueeze(-2)
               #print("k2 shape = ", k2.shape)
           # print("k shape = ", k.unsqueeze(1).shape, "q shape = ", q.transpose(-1, -2).shape)  
           # print("indexer q = ", q.to("cpu"), "k = ", k)
           # print("prefill q shape = ", q.shape, "k shape = ", k.shape, "q x :", q.transpose(-1, -2).shape, "k x shape ", k.unsqueeze(1).shape)
            logits = torch.matmul(k.unsqueeze(1), q.transpose(-1, -2))
           # relu = nn.ReLU(inplace=True)
            #logits = relu(logits)
            #print("logits 1==============", logits.to("cpu"))
            logits.clamp_min_(0)
         ...
         #   logits_cpu = logits.to("cpu")
          #  print("logits_cpu = ", logits_cpu)
 
        #    topk_indices_cpu = logits.topk(512, dim=-1) [1]
          ...
          #  topk_indices= topk_indices.to(dtype=torch.int32)
           ...
           # torch.hpu.synchronize()
            #print("decode key = ", k.shape, "cache = ", kv_cache.shape)
            k = k.view(-1, head_dim)
           # print("decode key2 = ", k.shape)
            key_cache = self.cache_op(k, kv_cache, block_indices, block_offsets)
            key_cache = kv_cache.unsqueeze(2)
            k = self.cache_op.fetch_from_cache(key_cache, block_list)#.unsqueeze(-2)
           # print("fetch k shape = ", k.shape, "key_cache shape = ", key_cache.shape)
            #print("decode fetch key shape = ", k.shape, "kv_cache shape = ", kv_cache.shape)
            #htorch.core.mark_step()
            #torch.hpu.synchronize()
           ...
           # print("q b2b shape = ", q.view(shape[0], -1).view(-1, *shape[1:]).shape)
            #print("q_cpu = ", q.to("cpu"))
            #print("q shape = ", q.shape, "attn_metadata.block_mapping shape = ", attn_metadata.block_mapping.shape)
            query = ops.batch2block(q, attn_metadata.block_mapping,
                        self.batch2block_matmul).unsqueeze(-2)
           # htorch.core.mark_step()
           # torch.hpu.synchronize()
 
            #print("query = :", query.to("cpu"))
           # print("q_cpu shape = ", q.shape, "query shape = ", query.shape)
 ...
 #           print("block mapping = ", attn_metadata.block_mapping)
  ...
  #              print("batch idx = ", batch_idx, "batch_block_list = ",batch_block_list)
   ...
   #             print("result shape = ", result.shape)
    ...
    def forward(
     ...
     #   print("q indexer 0 ", qr.to("cpu"))
        q, _ = self.wq_b(qr)
        q = q.view(-1, self.n_head, self.head_dim)
      #  print("q indexer 1 ========", q.to("cpu"))
    ...
    def forward( self,
        ...
        if self.q_lora_rank is not None:
            ...
            if self.indexer is not None:
              self.indexer(hidden_states, hidden_states_or_q_c, input_positions, self.rotary_emb, indexer_k_cache, attn_metadata)
          #  htorch.core.mark_step()
          #  torch.hpu.synchronize()
